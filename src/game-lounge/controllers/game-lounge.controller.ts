...
export class GameLoungeController { 
  ...
  @Post(':id/players/')
  async addUser(@Param('id') id: string,@Body() gameLoungeUserAddDto: GameLoungeUserAddDto) : Promise<GameLoungeUserDto> {
  ...
  @Get(':uid/players')
  async findAllGameLoungePlayers(@Param('uid') uid: string,): Promise<GameLoungeUserDto[]> {
    let gls:GameLoungeUserEntity[] = await this.gameLoungeService.findAllGlPlayers(uid);
  ...
  @Put(':id')
  async update(@Param('id') id: string,@Body(/*new ValidationPipe(VALIDATION_SCHEMA_GAMING_LOUNGE_UPDATE)*/) gamingLoungeUpdateDto: GamingLoungeUpdateDto) {
  ...
  @ApiParam({ name: "id", description: "Unique game lounge id generated by the system for the game lounge entities.", required: true, allowEmptyValue: false })
  @ApiResponse({ status: 200, description: 'Return game lounge found by the given id.'})
  @ApiResponse({ status: 400, description: 'Invalid id.' })
  @ApiResponse({ status: 404, description: 'Game lounge not found by the given id.' })
  @Get(':id')
  async findById(@Param('id') id: string): Promise<GameLoungeDto> {
