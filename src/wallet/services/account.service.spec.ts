...
const logger = new Logger(AccountService.name + + ".spec");
  ...
  beforeEach(async () => {
    ...
    const modRef = await Test.createTestingModule({
      ...
      providers: [
        ...
        {
          provide: getModelToken(AccountEntity ),
          useValue: {
            findAll: jest.fn(() => [accountXWGT]),
            findOne: jest.fn(),
            create: jest.fn(() => accountXWGT),
            remove: jest.fn(),
          }
        }
    ...
    model = modRef.get<typeof AccountEntity>(getModelToken(AccountEntity));
  });
